...
pub fn notify_error_with_cooldown(
    ...
    match status {
        ...
        Ok(exit) => warn!("cavaii: notify-send exited with status {exit}"),
        Err(err) => warn!("cavaii: could not invoke notify-send: {err}"),
    }
}
...
fn notifications_globally_disabled() -> bool {
    match std::env::var("CAVAII_DISABLE_NOTIFICATIONS") {
        Ok(value) => {
